...
namespace CREMOT.UIAnimatorDotween
{
    public class UIAnimator : MonoBehaviour
    {
        ...
        public class AnimationSettings
        {
            ...
            [SerializeField] private bool _canPlayOnPause;
            ...
            public bool CanPlayOnPause { get => _canPlayOnPause; set => _canPlayOnPause = value; }
        ...
        public void PlayAnimation(AnimationSettings settings)
        {
            switch (settings.AnimationType)
            {
                ...
                case EAnimationType.MOVETO_2:
                    settings.AnimationTween = transform.DOMove(settings.TargetMove.position, settings.Duration)
                        .SetUpdate(settings.CanPlayOnPause)
                        .SetEase(settings.Ease).OnComplete(()=> NotifyAnimationFinished(settings));
                    break;
                case EAnimationType.LOCALMOVETO_3:
                    settings.AnimationTween = transform.DOLocalMove(settings.TargetMove.localPosition, settings.Duration)
                        .SetUpdate(settings.CanPlayOnPause)
                        .SetEase(settings.Ease).OnComplete(() => NotifyAnimationFinished(settings));
                    break;
                case EAnimationType.SCALETO_4:
                    settings.AnimationTween = transform.DOScale(settings.TargetScale, settings.Duration)
                        .SetUpdate(settings.CanPlayOnPause)
                        .SetEase(settings.Ease).OnComplete(() => NotifyAnimationFinished(settings));
                    break;
                case EAnimationType.COLORTO_5:
                    settings.AnimationTween = AnimColorTo(settings.TargetColor, settings.Duration, settings.Ease, settings);
                    break;
                case EAnimationType.IDLE_INFINITE_6:
                    settings.AnimationTween = AnimateIdleInfinite(1f, settings.Duration, settings.Ease, settings);
                    break;
                case EAnimationType.BOBBING_ONCE_7:
        ...
        private Tween AnimateFade(float targetAlpha, float duration, Ease ease, AnimationSettings settings)
        {
            ...
            if (_canvasGroup != null)
            {
                return _canvasGroup.DOFade(targetAlpha, duration)
                    ...
                    .OnComplete(() => NotifyAnimationFinished(settings));
            }
            else if (_image != null)
            {
                return _image.DOFade(targetAlpha, duration)
                    .SetUpdate(settings.CanPlayOnPause)
                    .SetEase(settings.Ease)
                    .OnComplete(() => NotifyAnimationFinished(settings));
            }
            ...
            {
                return _spriteRenderer.DOFade(targetAlpha, duration)
                    .SetUpdate(settings.CanPlayOnPause)
                    .SetEase(settings.Ease)
                    .OnComplete(() => NotifyAnimationFinished(settings));
            }
        ...
        private Tween AnimateIdleInfinite(float direction, float duration, Ease ease, AnimationSettings settings)
        {
            ...
            return transform.DOLocalMoveY(settings.IdleAmplitude * direction, duration + tempOffset)
                .SetUpdate(settings.CanPlayOnPause)
                .SetEase(settings.Ease)
                .OnComplete(() => AnimateIdleInfinite(direction * -1f, duration, ease, settings));
        }
